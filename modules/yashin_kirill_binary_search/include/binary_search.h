...
#ifndef MODULES_YASHIN_KIRLL_BINARY_SEARCH_INCLUDE_BINARY_SEARCH_H_
...
class BinarySearch {
 public:
    ...
    int BinSearch(T key, std::vector<T> array, int left, int right) {
        ...
        if (right < left) {
            const std::string error_msg("Right border must be >= left border");
            throw error_msg;
        }
...
#endif  // MODULES_YASHIN_KIRLL_BINARY_SEARCH_INCLUDE_BINARY_SEARCH_H_
